...
\begin{abstract}
This paper describes a static type checker for a Forth(-like) language whose semantics is expressed as typed stack effects. The checker is implemented as a single Gforth program, but its core ideas are language independent: a finite type lattice is loaded from an external file, word specifications are given declaratively, and user-defined words are inferred by composing abstract stack transformations instead of executing the program. The central technical device is a collection of partial operations on stack effects. Sequential composition unifies adjacent stack symbols by taking the greatest lower bound of comparable types; branching uses a partial greatest-lower-bound operator on whole effects; and iteration is accepted only when repeated execution stabilizes to an idempotent effect. The implementation supports ordinary words, literals, parser words, defining words, and control structures such as \texttt{IF ... ELSE ... FI} and \texttt{BEGIN ... WHILE ... REPEAT}. We present the mathematical model, the operations implemented by the checker, and worked examples taken from the bundled specification and program files.
\end{abstract}
...
This paper sits at the intersection of several lines of work on stack-oriented typing. Early algebraic treatments of Forth stack effects emphasized sequential composition and its semigroup structure \cite{poial1994algebraic}. Typed wildcards and subtype-sensitive matching then made it possible to preserve both abstract type precision and symbolic identity across stack shuffles \cite{stoddart1993type,poial2002wildcards}. Later work reframed the analysis explicitly as must-analysis, motivating greatest lower bounds for branches and idempotents for loops \cite{poial2006typing}.

The present checker differs in emphasis. Rather than propagating explicit sets of alternatives through the whole analysis, it collapses alternatives eagerly to a single guaranteed effect via $\glb$. This loses may-information but keeps the domain small enough to support source-level parsing, user-defined words, and declaratively specified control structures in one implementation. In that respect the work is close in spirit to compositional type systems for stack-oriented low-level languages \cite{saabas2006compositional}, while staying within the source-level idiom of Forth stack effects and parser disciplines \cite{forth2012standard,power2004formal}.
...
\section{Conclusion}
The type checker described in this paper shows that this form of static analysis for a Forth(-like) language can be organized around a small mathematical core. A finite type lattice supplies precision information; indexed stack symbols express correlations; sequential composition gives a partial product on effects and, after totalization, a monoid with zero on the linear fragment; branch summaries are greatest lower bounds of effects; and loop summaries are accepted when repeated execution stabilizes to an idempotent effect. Because the checker loads both the type hierarchy and the word specifications from external files, the same implementation can be retargeted to other vocabularies and dialects with little or no change to the core.

The broader methodological point is that many of the dynamic-looking features of a concatenative language become statically tractable once they are phrased as operations in an effect algebra.
