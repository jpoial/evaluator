...
can construct a common contract.  It aligns their active suffixes and treats
extra deep cells shown by only one effect as an implicit unchanged prefix.
Inputs keep the more specific comparable type needed to make either path
valid; outputs keep the more general comparable type guaranteed after either
path.  A value identity is retained only when both paths establish it.  The
condition's \texttt{flag} consumption is then composed with that contract.
...
\end{lstlisting}
By contrast, paths with different net stack changes have no common usable
boundary and are rejected.  Differences in type can still be accepted when one
type is a declared subtype of the other.  On inputs the result records the
stronger requirement; on outputs it records the weaker guarantee that covers
both paths.  For example, \texttt{@ ( a-addr -- X )} and
\texttt{C@ ( c-addr -- char )} combine as
\texttt{( a-addr -- X )}, not \texttt{( a-addr -- char )}.
...
\texttt{MAYSWAP} cannot promise either output permutation, because the true
path swaps the cells and the false path preserves them; its common contract
therefore keeps only two generic output types.  Both loop examples preserve
their numeric input according to the one-versus-two rule.  \texttt{KEEPIF}
consumes a flag and duplicates the remaining cell regardless of the selected
branch.
...
\begin{itemize}
  ...
  \item Branch merging produces one variance-aware conservative contract, not
        a set of all possible branch effects; aligned types must still be
        directly comparable.
