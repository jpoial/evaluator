...
Let $\Types$ be a finite set of type names equipped with a precision order $\leq$. We read $\tau_1 \leq \tau_2$ as ``$\tau_1$ is at least as precise as $\tau_2$,'' matching the sample declarations
\begin{lstlisting}
rel a-addr < c-addr
rel c-addr < addr
rel addr < x
...
When the order is viewed as a finite lattice, this operation is the greatest lower bound on chains. Incomparable pairs have no usable meet for this checker and therefore cause a type clash.
...
\noindent
This proposition explains why the implementation may evaluate a sequence left to right: whenever every adjacent composition is defined, the final result does not depend on accidental wildcard names.
...
Loops require a stable summary of repeated execution. The checker does not attempt a general fixed-point engine. Instead it uses a compact algebraic criterion based on idempotence.
...
Thus the checker is not only typing tokens; it also models the source-level behavior needed to parse Forth-like syntax.

\subsection{Control structures}
Control structures are specified with paired \texttt{syntax:} and \texttt{effect:} blocks. For example, the sample repository contains:
\begin{lstlisting}
...
The implementation therefore supports the standard Forth workflow in which the specification dictionary grows during analysis. Inferred definitions are written to a log file; for the sample program the checker records:
\begin{lstlisting}
MYLOOP   ( flag[1] flag[1] -- flag[1] flag[1] )
MYCHOICE ( x a-addr flag -- )
...
\section{Conclusion}
The type checker described in this paper shows that a useful static analysis for a Forth(-like) language can be built from a very small mathematical core. A finite type lattice supplies precision information; indexed stack symbols express correlations; sequential composition forms an inverse semigroup of effects; branch summaries are greatest lower bounds of effects; and loop summaries are accepted when repeated execution stabilizes to an idempotent effect. Because the checker loads both the type hierarchy and the word specifications from external files, the same implementation can be retargeted to other vocabularies and dialects with little or no change to the core.
...
The type checker successfully analyzed its own source using Forth 2012 standard type system.
