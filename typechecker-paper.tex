...
Before composition, the implementation deep-clones the sequence of effects and
assigns each occurrence a disjoint wildcard range. The next range is computed
from the maximum after the current shift, matching the reference implementation's
allocation rule. Cloning is required because freshening and unification perform
sequence-wide substitutions: a prefix check must not mutate dictionary entries,
later checks, or the effect list eventually used for annotation.
...
\begin{proposition}
Modulo normalization of fresh indices, compatible effects form an inverse semigroup under $\compose$.
\end{proposition}
...
The checker therefore implements a greatest-lower-bound operation on whole effects, with the variance induced by effect contracts. Aligned inputs retain the more specific comparable type, so either branch can execute; aligned outputs retain the more general comparable type, so the result promises only what both branches guarantee. Output identities are compared as pairs of source identities and survive only when both alternatives establish the same correlation.

More concretely, if one effect has $k$ more inputs than another, it must also have $k$ more outputs. The shorter effect is interpreted as preserving an implicit $k$-cell deep prefix, and its active suffix is aligned at offset $k$ with the longer effect. If the depth differences disagree or any aligned type pair is incomparable, the branch summaries are incompatible. The implementation does not search for a third common type.
...
\]
The merged effect therefore preserves the stronger address requirement. Dually, merging \texttt{@ ( a-addr -- x )} with \texttt{C@ ( c-addr -- char )} yields \texttt{( a-addr -- x )}: inputs use the common subtype, while outputs use the common supertype.
...
\end{lstlisting}
which is the compact common interface: repetition preserves two flag-typed cells, while branch-dependent identities are not claimed.
...
Parser consumption and effect inference remain separate. In particular, text
inside an ordinary parenthesized comment is discarded even if it resembles a
stack effect, so \texttt{( -- n )} cannot replace inference of a definition
body. Only a recognized locals-style declaration contributes the provisional
input/output counts used while the body is being checked.
...
The Gforth and Java implementations also agree on successful presentation for
the checked-in positive corpus. The native reader normalizes LF, CRLF, and CR
terminators to the same logical lines as Java's buffered line reader, and the
native logger uses LF on Unix. Empty internal placeholder tokens are omitted
from annotations, which are printed from the effect clone normalized together
with the final result.

\section{Conclusion}
The type checker described in this paper shows that a useful static analysis for a Forth(-like) language can be built from a very small mathematical core. A finite type lattice supplies precision information; indexed stack symbols express correlations; sequential composition forms a partial semigroup of effects; branch summaries are variance-aware greatest lower bounds of effects; and loop summaries are accepted when repeated execution stabilizes to an idempotent effect. Because the checker loads both the type hierarchy and the word specifications from external files, the same implementation can be retargeted to other vocabularies and dialects with little or no change to the core.
